...
public class WalkIron extends Task {
    ...
    public WalkIron(ClientContext ctx) {
        super(ctx);
    }

    @Override
    public boolean activate() {
        return (ctx.inventory.select().id(440).count() == 0 && ctx.inventory.select().id(2351).count() > 0) || (ctx.inventory.select().id(440).count() == 28 && pathToBank[0].distanceTo(ctx.players.local()) > 6);
    }
